...
同一导入文件夹或 ZIP 包中，位于同一个一级功能目录的 Markdown 文件会被识别为同一功能模块。例如 `内测邀请码注册-v2/产品需求.md`、`内测邀请码注册-v2/开发阅读版.md` 和 `资料.zip!/内测邀请码注册-v2/负责人验收.md` 都会归入“内测邀请码注册”。

- 这些来源 PRD 会保留在 PRD 列表中，方便查看原始文件和分析记录；任务工作台只显示一个功能模块父级，并汇总来源数量、最新更新时间和任务数。
- 分析时会将该模块的所有来源文档合并为一次模型请求，避免产品、开发和验收说明各自产生重复任务。
- 从该模块任意一份来源 PRD 发起重新分配，都会刷新该模块的活动任务；`已完成`、`已取消`任务会保留历史记录。
- 已在旧版本中分别分析过的来源 PRD 会先显示在同一个功能模块父级下；从任一来源执行一次“重新分配”后，活动任务才会替换为该功能模块的一套去重任务。
- 删除一份来源 PRD 后，任务仍关联其他来源时会继续保留；只有删除该功能模块的最后一份来源 PRD，相关任务才会被删除。
...
- 删除 PRD：解除该 PRD 与任务的关联；若同一功能模块还存在其他来源 PRD，模块任务会保留。删除最后一个来源后，相关任务才会删除；历史调整知识继续保留。
