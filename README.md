...
- 同一导入文件夹或 ZIP 内同级功能目录的多份 PRD 会作为一个功能模块分析，工作台只显示一个功能父级；首次分析或重新分配会生成一套去重任务，每份来源 PRD 仍可追溯。
...
1. 打开应用，创建 PRD。
2. 导入文件、输入网页链接，或直接粘贴 PRD 内容；同一个文件夹或 ZIP 内的功能目录可包含多份互补说明。
3. 按需选择是否启用方案复核，然后提交分析。功能目录中的任一来源触发重新分配时，都会刷新该功能模块的活动任务。
...
- 删除 PRD 会解除它与任务的关联，但同一功能模块仍有其他来源 PRD 时，共享任务会保留；历史调整知识始终保留。
